...
batch = opts.val(1:numel(opts.val)) ;
[im, labels] = getBatch(imdb, batch) ;
printConfusionMatrix(opts, net, im, labels, res) ;
...
% -------------------------------------------------------------------------
function [] = printConfusionMatrix(opts, net, im, labels, res)
% -------------------------------------------------------------------------
...
for i=1:size(labels, 2),
  conf(predictions(1,1,1,i), labels(i)) = ...
      conf(predictions(1,1,1,i), labels(i)) + 1 ;
  if predictions(1,1,1,i) ~= labels(i),
      sprintf('Instance %d wrong: actual %d predicted %d\n', i, ...
          labels(i), predictions(1,1,1,i))
  end;
end;
